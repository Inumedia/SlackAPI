		...
		protected Uri GetSlackUri(string path, Tuple<string, string>[] getParameters)
		{
			string parameters = getParameters
				...
				.Select(new Func<Tuple<string, string>, string>(a =>
				{
					try
					{
						return $"{Uri.EscapeDataString(a.Item1)}={Uri.EscapeDataString(a.Item2)}";
					}
					catch (Exception ex)
					{
						throw new InvalidOperationException($"Failed when processing '{a}'.", ex);
					}
				}))
				.Aggregate((a, b) =>
				{
			...
			Uri requestUri = new Uri($"{path}?{parameters}");
			return requestUri;
		}
